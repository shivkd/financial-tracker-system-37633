...
from src.api.db import SessionLocal, get_password_hash, User, \
    Category, Transaction, Budget, get_user_by_email
...
async def create_category(cat: CategoryIn, current_user: User = Depends(get_current_user)):
    async with SessionLocal() as db:
...
async def update_category(category_id: int, cat: CategoryIn, current_user: User = Depends(get_current_user)):
    async with SessionLocal() as db:
...
async def create_transaction(tx: TransactionIn, current_user: User = Depends(get_current_user)):
    async with SessionLocal() as db:
        db_tx = Transaction(
...
async def update_transaction(transaction_id: int, tx: TransactionIn, current_user: User = Depends(get_current_user)):
    async with SessionLocal() as db:
...
async def set_budget(budget: BudgetIn, current_user: User = Depends(get_current_user)):
    async with SessionLocal() as db:
